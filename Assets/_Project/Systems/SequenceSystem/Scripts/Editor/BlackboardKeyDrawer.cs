...
namespace SequenceSystem.Editor
{
    ...
    public class BlackboardKeyDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var guids = AssetDatabase.FindAssets("t:BlackboardDefinitionsSO");

            if (guids.Length == 0)
            {
                // Fallback: If no config exists, just draw a normal text field with a warning
                EditorGUI.PropertyField(position, property, label);
                Debug.LogWarning("No 'BlackboardDefinitionsSO' found. Create one to use the Dropdown.");
                return;
            }

            string path = AssetDatabase.GUIDToAssetPath(guids[0]);
            var defs = AssetDatabase.LoadAssetAtPath<BlackboardDefinitionsSO>(path);
